...
{
public class ResTransportInputSystem : IBlahInitSystem, IBlahRunSystem
{
	private IBlahSignalProducer<ResTransportCmd> _resTransportCmd;
	...
	public void Run()
	{
		...
		{
			if (TryGetBizCloseToCursor(out var ent))
				_fromBizEnt = ent;
			else
				_fromBizEnt = null;
		}
		...
		{
			if (_fromBizEnt != null && 
			    TryGetBizCloseToCursor(out var ent) &&
			    _fromBizEnt != ent)
			{
				_resTransportCmd.AddFull(_fromBizEnt.Value, ent);
			}
			else
			{
				_fromBizEnt = null;
	...
	private bool TryGetBizCloseToCursor(out BlahEcsEntity bestEnt)
	{
		...
		foreach (var ent in _bizsFilter)
		{
			...
			if (biz.OwnerPlayer != _currPlayer)
				continue;
